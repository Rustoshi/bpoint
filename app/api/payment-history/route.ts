  ...
  try {
    ...
    for (const t of trades) {
      entries.push({
        ...
        description: `${t.brand} · ${t.currencySymbol ?? "$"}${t.cardValue ?? (t as { cardValueUSD?: number }).cardValueUSD ?? 0}`,
    ...
    for (const r of recoveries) {
      entries.push({
        ...
        description: `${r.brand} · ${r.currencySymbol ?? "$"}${r.cardValue ?? (r as { cardValueUSD?: number }).cardValueUSD ?? 0} card`,
        amountNGN: r.payoutNGN,
        amountFormatted: fmtNGN(r.payoutNGN),
        direction: "credit",
        status: tradeStatus[r.status] ?? r.status,
