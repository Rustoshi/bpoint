...
export async function POST(req: NextRequest) {
  try {
    ...
    const { lipsyncDescription, fileUrls, fileTypes, additionalNotes } = body;

    // ── Validate ──────────────────────────────────────────────────────────────
    if (!lipsyncDescription || String(lipsyncDescription).trim().length < 20) {
    ...
    if (!Array.isArray(fileUrls) || fileUrls.length === 0) {
      return NextResponse.json(
        { success: false, message: "Please upload at least one media file." },
        { status: 400 }
      );
    }
    ...
    const feeNGN = config.lipsyncFeeNGN;
    ...
    if (user.walletBalance < feeNGN) {
      return NextResponse.json(
        {
          success: false,
          message: `Insufficient wallet balance. You need ₦${feeNGN.toLocaleString()} to submit a lipsync request. Current balance: ₦${user.walletBalance.toLocaleString()}.`,
    ...
    const order = await LipsyncOrder.create({
      userId,
      lipsyncDescription: String(lipsyncDescription).trim(),
    ...
    return NextResponse.json({
      success: true,
      message: "Lipsync request submitted successfully.",
      orderId: order._id,
      feeChargedNGN: feeNGN,
    });
  } catch (error) {
    console.error("[POST /api/lipsync/submit]", error);
